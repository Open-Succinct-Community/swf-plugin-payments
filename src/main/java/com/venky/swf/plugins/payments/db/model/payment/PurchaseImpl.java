...
public class PurchaseImpl extends ModelImpl<Purchase> {
    ...
    public void refreshPaymentStatus(){
        ...
        if (!ObjectUtil.isVoid(purchase.getRazorpayPaymentId())){
            Payment payment ;
            try {
                payment = createRazorpayClient().payments.fetch(purchase.getRazorpayPaymentId());
            } catch (RazorpayException e) {
                throw new RuntimeException(e);
            }
            purchase.setStatus(payment.get("status"));
            if (Purchase.PaymentStatus.valueOf(purchase.getStatus()).ordinal()
